...
class GeHtml(object):
    ...
    def get_article(self,url,Blog):
        ...
        soup=BeautifulSoup(html,'lxml')
        if Blog == 'cnblogs':
            article_html=soup.find('div',id='cnblogs_post_body').contents
            title=soup.find('a',id='cb_post_title_url').text
            time=soup.find('span',id='post-date').text
            ...
            article_html=soup.find('div',id='article_content').contents
            title=soup.find('span',class_='link_title').text
            time=soup.find('span',class_='link_postdate').text
            category=''
            category_soup=soup.find('span',onclick="_gaq.push(['_trackEvent','function', 'onclick', 'blog_articles_fenlei']);")
            if not category_soup is None:
                category=category_soup.contents[0]
            tag=[]
            tag_tag=[]
            tag_soup=soup.find('span',class_="link_categories")
            if not tag_soup is None:
                tag_tag=tag_soup.find_all('a')
            for i in tag_tag:
                tag.append(i.text)
        else:
            article_html = soup.find('div', id='article_content').contents
            title=soup.find('h1',class_='csdn_top').text
            time=soup.find('span',class_='time').text
            category=""
            tag_div=[]
            tag_soup=soup.find('ul',class_="article_tags clearfix csdn-tracking-statistics tracking-click")
            if not tag_soup is None:
                tag_div=tag_soup.find_all('a')
            tag=[]
            for i in tag_div:
                tag.append(i.text)
        article=Article()
